...
const db = require('../db'); 
const { sendRequestAdoptionEmail, notifyMatchingAdopters } = require('../services/notificationService');
...
router.post('/search', async (req, res) => {
  ...
  router.post('/create', async (req, res) => {
    ...
    let { associationId, animalType, gender, name, breed, age, medicalHistory, residentialArea, description, encodedImages } = req.body;
    ...
    const insertQuery = `
        INSERT INTO Animals (
            AssociationID, AnimalType, Gender, Name, Breed, Age, MedicalHistory, ResidentialArea, description, encodedImages
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`;
    ...
    try {
      ...
      const insertResult = await db.query(insertQuery, [associationId, animalType, gender, name, breed, age, medicalHistory, residentialArea, description, encodedImages]);
      ...
      db.query(selectQuery, async function (err, result, fields) {
        ...
        if (result.length > 0) {
          ...
          const newAnimal = {  name, animalType, gender, breed, age, residentialArea, description, encodedImages,  animalId};
